#!/usr/bin/env python3
"""
Consistency Issues Fixer

This script fixes common consistency issues found in documentation files:
...
class ConsistencyFixer:
    """Fixes common consistency issues in documentation files."""
    ...
    def fix_broken_links(
        ...
        link_fixes = [
            (
                r"../reports/gui_test_coverage_analysis\.md",
                "gui_test_coverage_analysis.md",
            ),
            (
                r"../guides/specifications/checkpoint_format_specification\.md",
                "checkpoint_format_specification.md",
            ),
            (
                r"../../scripts/experiments/README\.md",
    ...
    def fix_import_format(self, content: str) -> tuple[str, int]:
        """Fix import format issues."""
        fixes_made = 0

        # Fix old src. imports in documentation reports
        if "from src." in content and "report" in content.lower():
            # These are documentation reports, so old imports are expected
            if self.verbose:
                logger.info("  Skipping import fix for documentation report")
            return content, fixes_made

        # Fix any remaining src. imports
        old_imports = re.findall(r"from\s+src\.", content)
        if old_imports:
            content = re.sub(r"from\s+src\.", "from crackseg.", content)
            fixes_made += len(old_imports)
            if self.verbose:
                logger.info(
                    f"  Fixed {len(old_imports)} old import statements"
                )

        return content, fixes_made

    def process_file(self, file_path: str) -> bool:
        """Process a single file for consistency fixes."""
        try:
            ...
            content, import_fixes = self.fix_import_format(content)
            total_fixes += import_fixes
            self.stats["import_fixes"] += import_fixes

            # Check if any changes were made
            if content != original_content:
                if self.backup and not self.dry_run:
                    backup_path = f"{file_path}.consistency_backup"
    ...
    def _print_summary(self) -> None:
        ...
        logger.info("")
        logger.info("=" * 60)
        logger.info("CONSISTENCY ISSUES FIXING SUMMARY")
        logger.info("=" * 60)
        ...
        logger.info(f"Import fixes: {self.stats['import_fixes']}")
...
def main():
    """Main function for consistency fixing."""
    parser = argparse.ArgumentParser(
        description="Fix common consistency issues in documentation"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
