...
{e}")
        ...
        summary_dict = {
            **base_info,
            ...
            "layer_hierarchy": get_layer_hierarchy(
                self.encoder, self.bottleneck, self.decoder,
    ...
    def print_summary(self, input_shape: Tuple[int, ...] = None,
        ...
        print(f"Trainable Parameters: {params['trainable']:,} "
              f"({params['trainable_percent']:.2f}%)", file=target_file)
        ...
        if "estimated_activation_mb" in mem:
            print(f"Estimated Activation Memory: "
                  f"{mem['estimated_activation_mb']:.2f} MB", file=target_file)
            print(f"Total Estimated Memory: "
                  f"{mem['total_estimated_mb']:.2f} MB", file=target_file)
        ...
        if "theoretical_rf_size" in rf:
            print(f"  Theoretical Size: {rf['theoretical_rf_size']}x"
                  f"{rf['theoretical_rf_size']}", file=target_file)
...
{summary_dict['final_activation_type']}",
                  file=target_file)
        print("\nLayer Hierarchy:", file=target_file)
        print("-" * 80, file=target_file)
        print(f"{'Layer':<25} {'Type':<20} {'Parameters':>12} "
              f"{'Input Ch':>10} {'Output Ch':>10}", file=target_file)
        print("-" * 80, file=target_file)
        for layer in summary_dict["layer_hierarchy"]:
            ...
            print(f"{layer_name:<25} {layer_type:<20} {params:>12,} "
                  f"{in_ch:>10} {out_ch:>10}", file=target_file)
            if "blocks" in layer:
                for block in layer["blocks"]:
                    ...
                    print(f"{block_name:<25} {block_type:<20} "
                          ...
                          f"{block_out_ch:>10}", file=target_file)
        print("-" * 80, file=target_file)
        print("=" * 80 + "\n", file=target_file)
        if return_string:
