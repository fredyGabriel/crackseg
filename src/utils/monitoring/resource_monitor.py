...
class ResourceMonitor:
    ...
    def _get_gpu_info(self) -> dict[str, float]:
        ...
        try:
            ...
            # Memory info
            memory_info = nml.nvmlDeviceGetMemoryInfo(handle)
            memory_used_mb = memory_info.used / (1024 * 1024)
            memory_total_mb = memory_info.total / (1024 * 1024)
            memory_percent = (memory_info.used / memory_info.total) * 100

            # Utilization
            util_info = nml.nvmlDeviceGetUtilizationRates(handle)
            utilization_percent = float(util_info.gpu)

            # Temperature
            temperature = nml.nvmlDeviceGetTemperature(
                handle, nml.NVML_TEMPERATURE_GPU
            )
