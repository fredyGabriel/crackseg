...
Examples:
    ...
    ```python
    from src.data.dataloader import create_dataloader, DataLoaderConfig
...
def _calculate_adaptive_batch_size(
    ...
    """
    Calculate optimal batch size based on available GPU memory to prevent OOM.

    This function implements intelligent batch size adaptation that monitors
    available GPU memory and adjusts the batch size to maximize utilization
    while preventing out-of-memory errors during training.

    The adaptive algorithm:
    1. Checks if CUDA is available and adaptation is enabled
    2. Queries current available GPU memory with safety margin
    3. Estimates memory per sample (rough heuristic, needs refinement)
    4. Calculates maximum possible batch size within memory constraints
    5. Returns the minimum of requested and memory-limited batch size

    Args:
        current_batch_size: Originally requested batch size.
        adaptive_enabled: Whether to enable adaptive batch sizing.
            If False, returns current_batch_size unchanged.
        max_memory_limit_mb: Optional maximum memory limit in MB.
            If specified, limits memory usage even if more is available.

    Returns:
        int: Adapted batch size that fits within memory constraints.
            Always returns at least 1, even if memory is very limited.

    Examples:
        Basic adaptation:
        ```python
        # With 8GB available, might reduce batch size from 64 to 32
        adapted = _calculate_adaptive_batch_size(64, True, None)
        ```

        With memory limit:
        ```python
        # Limit to 4GB even if more is available
        adapted = _calculate_adaptive_batch_size(32, True, 4000.0)
        ```

        Disabled adaptation:
        ```python
        # Returns original batch size unchanged
        adapted = _calculate_adaptive_batch_size(32, False, None)
        assert adapted == 32
        ```

    Note:
        The current implementation uses a rough heuristic for memory per
        sample. Future versions should implement more accurate memory
        estimation based on actual model and data characteristics.

        Warnings are issued when batch size is automatically reduced to
        help users understand performance implications.
    """
...
def _configure_num_workers(requested_num_workers: int) -> int:
    ...
    requested, or validates the manually specified worker count.

    Worker count strategy:
    - Manual specification (>= 0): Use the provided value directly
    - Auto-detection (-1): Use half of available CPU cores as heuristic
    - Fallback: Default to 1 worker if CPU count cannot be determined

    The heuristic of using half the CPU cores balances:
    - Data loading throughput: More workers load data in parallel
    - System resource usage: Leaves cores available for training
    - Memory consumption: Each worker uses additional memory

    Args:
        requested_num_workers: Requested number of workers.
            Use -1 for automatic detection based on CPU cores.
            Use 0 for no multiprocessing (main process only).
            Use positive integers for specific worker counts.

    Returns:
        int: Actual number of workers to use. Always >= 0.

    Examples:
        Automatic detection:
        ```python
        # On 8-core system, returns 4 workers
        workers = _configure_num_workers(-1)
        ```

        Manual specification:
        ```python
        # Returns exactly 6 workers
        workers = _configure_num_workers(6)
        assert workers == 6
        ```
        ...
        workers = _configure_num_workers(0)
        assert workers == 0
        ```

    Note:
        The half-CPU-cores heuristic works well for most scenarios but may
        need adjustment for specific hardware configurations:
        - High-core count systems: May want to limit workers further
        - Memory-constrained systems: Consider fewer workers
        - Storage-limited systems: More workers may not help

        Warnings are issued if CPU count detection fails to help with
...
def _create_sampler_from_config(
    ...
    Args:
        ...
        shuffle_param: Original shuffle setting from DataLoader config.
