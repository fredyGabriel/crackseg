...
validate_ci_config() {
    ...
    if ! docker-compose -f "$SCRIPT_DIR/../docker-compose.test.yml" config > /dev/null 2>&1; then
    ...
    if [[ -f "$ci_compose" ]]; then
        if ! docker-compose -f "$SCRIPT_DIR/../docker-compose.test.yml" -f "$ci_compose" config > /dev/null 2>&1; then
