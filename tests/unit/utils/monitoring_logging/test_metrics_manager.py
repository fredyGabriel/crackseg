...
class TestMetricLogging:
    ...
    def test_log_training_metrics_with_tensors(
        ...
        """Test logging metrics with tensor inputs."""
        metrics: dict[str, float | torch.Tensor] = {
            "loss": 0.5,
            "accuracy": 0.85,
            "iou": torch.tensor(0.72),
        }
        ...
        assert entry["metrics"]["iou"] == pytest.approx(0.72, rel=1e-6)
...
class TestMetricRetrieval:
    ...
    def test_get_metric_history(self, metrics_manager: MetricsManager) -> None:
        ...
        assert history[2][1] == pytest.approx(0.2, rel=1e-6)
...
class TestMetricFormatting:
    ...
    def test_extract_scalar_metrics_mixed_types(
        ...
        # Use tolerance for floating point comparison (mean of [0.6, 0.8])
        assert scalar_metrics["tensor_multi"] == pytest.approx(0.7, rel=1e-6)
