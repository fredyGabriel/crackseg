"""
Storage management for capture artifacts. This module provides storage
utilities for managing screenshots, videos, and visual regression
artifacts with configurable retention policies, naming conventions,
...
class StorageConfig:
    """
    Configuration for capture storage management. Attributes: base_dir:
    Base directory for all capture artifacts screenshots_dir: Subdirectory
    for screenshots videos_dir: Subdirectory for videos regression_dir:
    Subdirectory for visual regression artifacts temp_dir: Temporary
    directory for processing retention_policy: Policy for cleaning up old
    artifacts retention_days: Days to keep artifacts (for KEEP_RECENT
    policy) max_files_per_test: Maximum files per test (0 = unlimited)
    naming_convention: File naming convention compress_old_files: Compress
    files older than threshold compression_threshold_days: Days after
    which to compress files auto_cleanup: Enable automatic cleanup on
    startup preserve_failure_artifacts: Always preserve artifacts from
    failed tests
    """
...
class CaptureStorage:
    """
    Storage management for capture artifacts. Handles file organization,
    retention policies, cleanup, and compression for screenshots, videos,
    and visual regression artifacts.
    """

    def __init__(self, config: StorageConfig | None = None) -> None:
        """
        Initialize storage management with configuration. Args: config:
    ...
    def store_artifact(
        ...
        """
        Store an artifact with proper naming and organization. Args:
        file_path: Path to the source file artifact_type: Type of artifact
        (screenshot, video, regression) test_name: Name of the test that
        generated this artifact metadata: Optional metadata to store with the
        artifact Returns: Path to the stored artifact Raises:
        FileNotFoundError: If source file doesn't exist OSError: If storage
        operation fails
        """
    ...
    def cleanup_artifacts(
        ...
        """
        Clean up old artifacts according to retention policy. Args: test_name:
        Specific test to clean (None for all tests) force: Force cleanup
        regardless of retention policy Returns: Number of files cleaned up
        """
        cleaned_count = 0
    ...
    def get_artifact_info(self, test_name: str) -> dict[str, Any]:
        """
        Get information about artifacts for a specific test. Args: test_name:
        Name of the test Returns: Dictionary with artifact information
        """
        info: dict[str, Any] = {
            "test_name": test_name,
            "artifacts": {},
