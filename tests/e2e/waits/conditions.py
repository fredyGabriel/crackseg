"""
Custom wait conditions for E2E testing. This module provides custom
wait conditions specifically designed for Streamlit applications and
general web testing scenarios. Includes both individual condition
functions and condition factory classes.
"""
...
def element_text_matches(
    ...
    """
    Wait condition for element text to match pattern. Args: locator:
    Selenium locator tuple pattern: Text pattern to match regex: Whether
    to use regex matching Returns: Condition function for WebDriverWait
    """
...
def element_count_equals(
    ...
    """
    Wait condition for specific number of elements. Args: locator:
    Selenium locator tuple expected_count: Expected number of elements
    Returns: Condition function returning elements if count matches
    """
...
def element_attribute_contains(
    ...
    """
    Wait condition for element attribute to contain value. Args: locator:
    Selenium locator tuple attribute: HTML attribute name value: Expected
    value in attribute Returns: Condition function for WebDriverWait
    """
...
def text_to_be_present_in_element_value(
    ...
    """
    Wait condition for text in element value attribute. Args: locator:
    Selenium locator tuple text: Text to check for in value Returns:
...
class CustomConditions:
    """
    Factory class for custom wait conditions. Provides reusable wait
    conditions for common testing scenarios that are not covered by
    Selenium's built-in expected_conditions.
    """
    ...
    def element_to_be_stale(
        ...
        """
        Wait for element to become stale (removed from DOM). Args: element:
        WebElement to check for staleness Returns: Condition function for
        WebDriverWait
        """
    ...
    def element_attribute_to_be_updated(
        ...
        """
        Wait for element attribute value to change. Args: locator: Selenium
        locator tuple attribute: Attribute name to monitor initial_value:
        Initial value to compare against Returns: Condition function for
        WebDriverWait
        """
    ...
    def page_title_matches(
        ...
        """
        Wait for page title to match pattern. Args: pattern: Title pattern to
        match regex: Whether to use regex matching Returns: Condition function
        for WebDriverWait
        """
...
class StreamlitConditions:
    """
    Streamlit-specific wait conditions. Provides wait conditions
    specifically designed for testing Streamlit applications, handling app
    lifecycle, widgets, and dynamic content loading.
    """
    ...
    def app_ready(
        ...
        """
        Wait for Streamlit app to be fully ready. Args: check_sidebar: Whether
        to check for sidebar presence additional_elements: Additional CSS
        selectors to check Returns: Condition function for WebDriverWait
        """
    ...
    def sidebar_loaded() -> Callable[[WebDriver], bool]:
        """
        Wait for Streamlit sidebar to be loaded and visible. Returns:
        Condition function for WebDriverWait
        """
    ...
    def no_spinners_present() -> Callable[[WebDriver], bool]:
        """
        Wait for all Streamlit loading spinners to disappear. Returns:
        Condition function for WebDriverWait
        """
    ...
    def file_upload_complete(
        ...
        """
        Wait for file upload to complete in Streamlit. Args: uploader_locator:
        Optional specific file uploader locator Returns: Condition function
        for WebDriverWait
        """
    ...
    def rerun_complete(
        ...
        """
        Wait for Streamlit rerun to complete. Args: max_wait_for_spinner: Max
        time to wait for spinner to appear Returns: Condition function for
        WebDriverWait
        """
    ...
    def session_state_contains(
        ...
        """
        Wait for session state to contain specific key/value. Args: key:
        Session state key to check expected_value: Expected value (None means
        just check key exists) Returns: Condition function for WebDriverWait
        """
...
"""
                ...
                if expected_value is not None:
                    script = f"""
return window.streamlitSessionState &&
window.streamlitSessionState['{key}'] === arguments[0];
"""
