"""Streamlit-specific session state management utilities.

This module provides specialized session state management for Streamlit
applications, including integration with Streamlit's session_state mechanism,
app-specific state validation, and CrackSeg application utilities.
"""
...
class StreamlitSessionManager:
    ...
    state including integration with st.session_state, page-specific state
    validation, and CrackSeg application workflows.
    """
    ...
    def get_streamlit_session_state(
        ...
        """Get Streamlit session_state using JavaScript.
    ...
    def set_streamlit_session_value(
        ...
        try:
            ...
            if (window.parent && window.parent.sessionStorage) {
                var streamlitSessionState = JSON.parse(
                    window.parent.sessionStorage.getItem(
                        'streamlit_session_state'
                    ) || '{}'
                );
                streamlitSessionState[arguments[0]] = JSON.parse(arguments[1]);
                window.parent.sessionStorage.setItem(
                    'streamlit_session_state',
                    JSON.stringify(streamlitSessionState)
                );
                return true;
            }
            return false;
    ...
    def remove_streamlit_session_value(
        ...
        """Remove a value from Streamlit session_state.
        ...
        Returns:
            True if value was removed successfully
        """
        try:
            script = """
            if (window.parent && window.parent.sessionStorage) {
                ...
                if (arguments[0] in streamlitSessionState) {
                    delete streamlitSessionState[arguments[0]];
                    window.parent.sessionStorage.setItem(
                        'streamlit_session_state',
                        JSON.stringify(streamlitSessionState)
                    );
    ...
    def clear_streamlit_session_state(
        ...
        """Clear all Streamlit session_state.
        ...
        Returns:
            True if session state was cleared successfully
        """
        try:
            script = """
            if (window.parent && window.parent.sessionStorage) {
    ...
    def capture_crackseg_app_snapshot(
        ...
        """Capture CrackSeg application state snapshot.

        Args:
            driver: WebDriver instance
            include_config: Whether to include configuration state
            include_model: Whether to include model state
            include_training: Whether to include training state

        Returns:
            Dictionary containing application state snapshot
        """
        try:
            session_state = self.get_streamlit_session_state(driver)
    ...
    def wait_for_streamlit_state_change(
        ...
        """Wait for a Streamlit session state change.

        Args:
            driver: WebDriver instance
            key: Session state key to monitor
            expected_value: Expected value (any change if None)
            timeout: Maximum time to wait
            poll_interval: How often to check state

        Returns:
            True if state change detected
        """
        try:
    ...
    def reset_crackseg_app_state(
        ...
        """Reset CrackSeg application state to defaults.

        Args:
            driver: WebDriver instance
            preserve_config: Whether to preserve configuration state
            preserve_model: Whether to preserve model state
    ...
    def cache_app_state(self, snapshot: dict[str, Any], key: str) -> None:
        """Cache an application state snapshot.

        Args:
            snapshot: State snapshot to cache
            key: Cache key
        """
    ...
    def get_cached_app_state(self, key: str) -> dict[str, Any] | None:
        """Get a cached application state snapshot.
