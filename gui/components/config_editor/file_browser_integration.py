"""
File browser integration for YAML configuration editor. This module
provides file discovery, loading, and browser functionality for the
...
class FileBrowserIntegration:
    ...
    def _render_file_item(self, file_path: Path, key: str) -> None:
        """
        Render individual file item with metadata and actions. Args:
        file_path: Path to the configuration file key: Base key for the
        browser component
        """
    ...
    def _load_config_file(self, file_path: Path) -> None:
        """
        Load configuration file into the editor. Args: file_path: Path to the
        configuration file
        """
        try:
    ...
    def render_advanced_load_dialog(self, key: str) -> None:
        """
        Render advanced file loading dialog with common files. Args: key: Base
        key for the editor component
        """
    ...
    def _load_file_by_path(self, file_path: str, key: str) -> None:
        """
        Load file by path with error handling. Args: file_path: Path to the
        file to load key: Base key for the editor component
        """
        try:
    ...
    def _render_quick_load_buttons(self, key: str) -> None:
        """
        Render quick load buttons for common files. Args: key: Base key for
